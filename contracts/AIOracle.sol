...
contract AIOracle is IAIOracle {
    ...
    // Define the DA enum 
    enum DA {
        OnChain,  // Data stored on-chain
        IPFS,     // Data on IPFS
        Arweave,   // Data on Arweave (example)
        Calldata
    }

     // Define an enum for error data
    enum AIOracleError {
         None,
         InsufficientFee,
         InvalidInput
    ...
    event Confirm(address indexed sender, uint256 requestId, bytes32 outputHash);
    ...
    constructor(address _opml) {
        opml = IOpml(_opml); // Initialize in constructor
    }
